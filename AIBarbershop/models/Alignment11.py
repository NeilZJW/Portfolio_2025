...
class Alignment(nn.Module):
    def __init__(self, opts, net=None):
        ...
        self.load_segmentation_net()
        self.load_downsampling()
        self.build_align_loss_builder()

    def load_segmentation_net(self):
    ...
    def build_align_loss_builder(self):
        self.loss_builder = AlignLossBuilder(self.opts)

    def processing_img(self, img_path):
        img = tv.transforms.ToTensor()(
            Image.open(img_path)
        )[:3].unsqueeze(0).to(self.opts.device)
        img = (self.downsample(img).clamp(0, 1) - seg_mean) / seg_std
        return img

    def build_align_optimizer(self, latent_path=None):
        if latent_path:
            latent_W = torch.from_numpy(
                convert_npy_code(np.load(latent_path))
            ).to(self.opts.device).requires_grad_(True)
        else:
            latent_W = self.net.latent_avg.reshape(
                1, 1, 512
            ).repeat(1, 18, 1).clone().detach().to(self.opts.device).requires_grad_(True)
        ...
        opt_align = opt_dict[self.opts.opt_name](
            [latent_W], lr=self.opts.learning_rate
        )
        return opt_align, latent_W
    ...
    def create_tar_seg_mask(self, img_path1, img_path2, sign, save_intermediate=True):
        device = self.opts.device

        im1 = self.processing_img(img_path1)
        down_seg1, _, _ = self.seg(im1)
        seg_tar1 = torch.argmax(down_seg1, dim=1).long()
        ggg = torch.where(
            seg_tar1 == 0,
            torch.zeros_like(seg_tar1),
            torch.ones_like(seg_tar1)
        )
        ...
        seg_tar1 = seg_tar1[0].byte().cpu().detach()
        seg_tar1 = torch.where(
            seg_tar1 == 10,
            torch.zeros_like(seg_tar1),
            seg_tar1
        )

        im2 = self.processing_img(img_path2)
        down_seg2, _, _ = self.seg(im2)
        seg_tar2 = torch.argmax(down_seg2, dim=1).long()
        ggg = torch.where(
            seg_tar2 == 10,
            torch.ones_like(seg_tar2),
            ggg
        )
        hair_mask2 = torch.where(
            seg_tar2 == 10,
            torch.ones_like(seg_tar2),
            torch.zeros_like(seg_tar2)
        )
        seg_tar2 = seg_tar2[0].byte().cpu().detach()
        OB_region = torch.where(
            (seg_tar2 != 10) * (seg_tar2 != 0) *
            (seg_tar2 != 15) * (seg_tar1 == 0),
            255 * torch.ones_like(seg_tar1),
            torch.zeros_like(seg_tar1)
        )

        new_tar = torch.where(
            seg_tar2 == 10,
            10 * torch.ones_like(seg_tar1),
            seg_tar1
        )
        inpainting_region = torch.where(
            (new_tar != 0) * (new_tar != 10),
            255 * torch.ones_like(new_tar),
            OB_region
        ).numpy()
        tmp = torch.where(
            torch.from_numpy(inpainting_region) == 255,
            torch.zeros_like(new_tar),
            new_tar
        ) / 10
        new_tar_inpainted = cv2.inpaint(
            tmp.clone().numpy(), inpainting_region, 3, cv2.INPAINT_NS
        ).astype(np.uint8) * 10

        new_tar_final = torch.where(
            OB_region,
            torch.from_numpy(new_tar_inpainted),
            new_tar
        )
        target_mask = new_tar_final.unsqueeze(0).long().cuda()

        ###### add auto-inpainting ######
        optimizer_align, latent_align = self.build_align_optimizer()
        latent_end, = latent_align[:, 6:, :].clone().detach()
        pbar = tqdm(range(80), desc="Create Target Mask Step1", leave=False)
        for step in pbar:
            optimizer_align.zero_grad()
            latent_in = torch.cat(
                [latent_align[:, :6, :], latent_end],
                dim=1
            )
        ...
        previous_target_mask = target_mask.clone().detach()
        ...
        optimizer_align, latent_align = self.build_align_optimizer()
        latent_end = latent_align[:, 6:, :].clone().detach()
        pbar = tqdm(range(80), desc="Create Target Mask Step 2", leave=False)
        for step in pbar:
            optimizer_align.zero_grad()
            latent_in = torch.cat(
                [latent_align[:, :6, :], latent_end],
        ...
        hair_mask_target = F.interpolate(
            hair_mask_target.float().unsqueeze(0),
            size=(512, 512),
            mode="nearest"
        )
    ...
    def dilate_erosion(self, free_mask, device, dilate_erosion=5):
        ...
        free_mask_D, free_mask_E = cuda_unsqueeze(
            dilate_erosion_mask_tensor(free_mask, dilate_erosion=dilate_erosion),
            device
        )
    ...
    def align_img(
        ...
        latent_FS_path_1 = os.path.join(
            output_dir, "FS", "{}.npz".format(im_name_1)
        )
        latent_FS_path_2 = os.path.join(
            output_dir, "FS", "{}.npz".format(im_name_2)
        )
        latent_1, latent_F_1 = load_FS_latent(latent_FS_path_1, device)
        latent_2, latent_F_2 = load_FS_latent(latent_FS_path_2, device)
        latent_W_1 = os.path.join(
            output_dir, "W+", "{}.npy".format(im_name_1)
        )
        latent_W_2 = os.path.join(
            output_dir, "W+", "{}.npy".format(im_name_2)
        )
        optimizer_align, latent_align_1 = self.build_align_optimizer(latent_W_1)
        ...
        optimizer_align, latent_align_2 = self.build_align_optimizer(latent_W_2)
        with torch.no_grad():
            tmp_latent_in = torch.cat(
                [latent_align_2[:, :6, :], latent_2[:, 6:, :]],
                dim=1
            )
            down_seg_tmp, I_S_Style_changed = self.create_down_seg(tmp_latent_in)
            ...
            HM_Structure = F.interpolate(
                HM_Structure.float().unsqueeze(0),
                size=(256, 256),
                mode="nearest"
            )
        ...
        for step in pbar:
            ...
            loss = ce_loss
            # Style Loss
            H1_region = self. downsample256(I_S_Style_changed) * HM_Structure
            H2_region = self.downsample256(gen_im) * HM_G
            style_loss = self.loss_builder.style_loss(
                H1_region, H2_region,
                mask1=HM_Structure, mask2=HM_G
            )
        ...
        self.save_align_results(
            im_name_1, im_name_2, sign, gen_im,
            latent_1, latent_F_mixed,
            save_intermediate=save_intermediate
        )
        return all_loss / iters

    def save_align_results(
            self, im_name1, im_name2, sign, gen_im,
            latent_in, latent_F,
            save_intermediate=True
    ):
        save_im = toPIL(
        ...
        save_dir = os.path.join(
            self.opts.output_dir, "Align_{}".format(sign)
        )
        os.makedirs(save_dir, exist_ok=True)
        latent_path = os.path.join(
            save_dir, "{}_{}.npz".format(im_name1, im_name2)
        )
        if save_intermediate:
            img_path = os.path.join(
                save_dir, "{}_{}.png".format(im_name1, im_name2)
            )
