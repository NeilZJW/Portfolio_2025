...
from utils.PCA_utils import IPCA
...
class Net(nn.Module):
    ...
    def load_weights(self):
        if not os.path.exists(self.opts.ckpt):
            print(
                "Downloading StyleGANv2 checkpoint: {}".format(
                    self.opts.ckpt
                )
            )
            download_weight(self.opts.ckpt)
        print(
            "Loading StyleGANv2 from checkpoint: {}".format(
                self.opts.ckpt
            )
        )
    ...
    def build_PCA_model(self, PCA_path):
        with torch.no_grad():
            ...
            pulse_space = torch.nn.LeakyReLU(5)(
                self.generator.style(latent)
            ).numpy()
            self.generator.style.to(self.opts.device)


        transformer = IPCA(512)
        X_mean = pulse_space.mean(0)
        transformer.fit(pulse_space - X_mean)
        X_comp, X_stdev, X_var_ratio = transformer.get_component()
        np.savez(
            PCA_path,
            X_mean=X_mean, X_comp=X_comp,
    ...
    def load_PCA_model(self):
        ...
        self.X_mean = torch.from_numpy(
            PCA_model["X_mean"]
        ).float().to(device)
        self.X_comp = torch.from_numpy(
            PCA_model["X_comp"]
        ).to(device)
