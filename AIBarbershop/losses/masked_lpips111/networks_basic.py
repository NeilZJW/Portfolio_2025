...
def spatial_average(in_tens, mask=None, keepdim=True):
    if mask is None:
        ...
        in_tens = in_tens.mean([2, 3], keepdim=keepdim)
        in_tens /= torch.sum(mask)
        return in_tens
...
class PNetLin(nn.Module):
    def __init__(
        ...
        elif self.pnet_type in ["alex"]:
            net_type = pn.AlexNet
            self.chns = [64, 192, 384, 256, 256]
        elif self.pnet_type in ["squeeze"]:
        ...
        if lpips:
            ...
            if self.pnet_type == "squeeze":
                ...
                self.lins.append(self.lin5)
                self.lin6 = NetLinLayer(self.chns[6], use_dropout=use_dropout)
                self.lins.append(self.lin6)
    ...
    def forward(self, in0, in1, mask=None, retPerLayer=False):
        ...
        for l in range(self.L):
            ...
            # check if (l + 1), i.e., [1 -> 5] in self.blocks,
            # then count the loss
...
class Dist2LogitLayer(nn.Module):
    """
    takes 2 distances, puts through fc layers,
    spits out value between [0,1] (if use_sigmoid is True)
    """
...
class L2(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if self.colorspace == "RGB":
            ...
            value = util.l2(
                util.tensor2np(
                    util.tensor2tensorlab(in0.data, to_nrom=False)
                ),
                util.tensor2np(
                    util.tensor2tensorlab(in1.data, to_nrom=False)
                ),
                r=100.
            ).astype("float")
