...
def score_2afc_dataset(data_loader, func, name=""):
    ...
    for data in tqdm(data_loader.load_data(), desc=name):
        ...
        d1s = np.array(d1s)
        gts = np.array(gts)
        scores = (d0s < d1s) * (1. - gts) + (d1s < d0s) * gts + (d0s == d1s) * .5
...
def score_jnd_dataset(data_loader, func, name=""):
    ...
    for data in tqdm(data_loader.load_data(), desc=name):
        ...
        gts += func(data["judge"]).cpu().numpy().tolist()
...
class DistModel(BaseModel):
    ...
    def initialize(
        ...
        if self.model == "net-lin":
            self.net = networks.PNetLin(
                ...
                pnet_type=net, use_dropout=True, spatial=spatial,
                version=version, lpips=True, vgg_blocks=vgg_blocks
            )
            kw = {}
            if not use_gpu:
                kw["map_location"] = "cpu"
            ...
            if not is_train:
                ...
                self.net.load_state_dict(torch.load(model, **kw), strict=False)
        ...
        if self.is_train:
            ...
            self.lr = lr
            self.old_lr = lr
            self.optimized_net = torch.optim.Adam(
        ...
        if printNet:
            print(" Networks Initialized ".center(50, "-"))
    ...
    def optimize_parameters(self):
        ...
        self.forward_train()
        self.backward_train()
        self.optimized_net.zero_grad()
        self.optimized_net.step()
    ...
    def update_learning_rate(self, n_epoch_decay):
        lrd = self.lr / n_epoch_decay
        lr = self.old_lr - lrd

        for param_group in self.optimized_net.param_groups:
            param_group["lr"] = lr
        print("Updating lr [%s] decay: %f -> %f" % (type, self.old_lr, lr))
        self.old_lr = lr
