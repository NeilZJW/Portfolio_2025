...
def get_landmark(filepath, predictor):
    ...
    lms = [
        np.array(
            [[tt.x, tt.y] for tt in shape.parts()]
        ) for shape in shapes
    ]
    return lms
...
def align_face(filepath, predictor):
    ...
    for lm in lms:
        ...
        eye_avg = (eye_right + eye_left) * 0.5
        eye_to_eye = eye_right - eye_left
        mouth_left = lm_mouth_outer[0]
        mouth_right = lm_mouth_outer[6]
        mouth_avg = (mouth_right + mouth_left) * 0.5
        ...
        # Crop.
        border = max(int(np.rint(qsize * 1)), 3)
        crop = (
        ...
        if enable_padding and max(pad) > border - 4:
            ...
            img += (
                np.median(img, axis=(0, 1)) - img
            ) * np.clip(mask, 0.0, 1.0)
            img = Image.fromarray(
                np.uint8(np.clip(np.rint(img), 0, 255)),
                "RGB"
            )
            quad += pad[:2]
